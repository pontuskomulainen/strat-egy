...
`CLAUDE.md` describes **how the harness works** — labels, workflows, gates, ratchets.
This file holds **the rules it enforces**: layer boundaries, the determinism rules for
`src/sim/**`, agent conduct, and the decisions that are not up for renegotiation. It is
the previous `CLAUDE.md`, unchanged except where a section already lives in the new one.
...
`npm run typecheck`, `npm run lint` and `npm test` exist today and are documented in
`CLAUDE.md`. Two more are specified here but are **not defined in `package.json` yet**:
